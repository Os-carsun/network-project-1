...
void showWelcomeMessage(int fd) {
...
void initalhints(struct addrinfo* hints) {
...
int checkAndRemoveCRLF(char **inupt, int size) {
    char* tmp = *inupt;
    int haveCRLF = 0;
...
void handleInput(int socket) {
    ...
    char* inputData = (char*) malloc(sizeof(char) * 1);
    int dataSize = 0;
    char* buffer = NULL;
    size_t a;
    char*** cmd=NULL;
    ...
    while(1){
        ...
        inputData = (char*)realloc(inputData, sizeof(char)* (dataSize + 1) );
        if(dataSize == (int)strlen(buffer)) strcpy(inputData, buffer);
